...
#Irina's comment: the code below is wrong, it says LASA not LUCAS, I fixed it below
#opal.assign.table.tibble(erasmus_opal,'LUCAS_Wave2','LASA.LUCAS_Wave2', variables=list())

opal.assign.table.tibble(erasmus_opal,'LUCAS_Wave2','LUCAS.LUCAS_Wave2', variables=list('id'))

#Irina's comment: I think this is also wrong: should have same name 
#LUCAS2 <- opal.execute(erasmus_opal,'LUCAS_Wave2')

LUCAS_Wave2 <- opal.execute(erasmus_opal,'LUCAS_Wave2')


```
## Assign participant id as first column of your harmonized dataframes
```{r,echo=FALSE}
#again had to fix code 
#env_LUCAS_0 <- data.frame(id= LUCAS2$id)
env_LUCAS_0 <- data.frame(id= LUCAS_Wave2$id)
